        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-3 group no-underline" style={{ textDecoration: 'none' }}>
            ...
            <div className="flex flex-col">
              <h1 className="text-base font-display font-semibold tracking-tight leading-none text-black" style={{ fontWeight: 600 }}>RAJ AI UI</h1>
              <span className="text-[10px] font-display font-normal text-neutral-400 tracking-wider uppercase leading-none" style={{ fontWeight: 400 }}>Component Engine</span>
            </div>
          </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`px-5 py-2.5 rounded-full text-sm font-display transition-all duration-300 flex items-center gap-2 touch-manipulation no-underline ${
                    active
                      ? 'bg-neutral-900 text-white shadow-lg font-medium'
                      : 'text-neutral-500 hover:text-black hover:bg-white/80 font-normal'
                  }`}
                  style={{ fontWeight: active ? 500 : 400, textDecoration: 'none' }}
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[10px] font-display tracking-wide transition-colors duration-300 ${active ? 'text-orange-500' : 'text-neutral-400'}`} style={{ fontWeight: active ? 500 : 400 }}>
                  {item.label}
                </span>
              </Link>
            )
