...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#faf8f6] text-black selection:bg-orange-500/10 selection:text-orange-600">
      <Suspense fallback={null}>
        <GLSLHills />
      </Suspense>
      <nav className="sticky top-0 z-[100] w-full px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 md:py-3.5">
        <div className="max-w-5xl mx-auto glass rounded-xl md:rounded-2xl px-3 sm:px-4 md:px-5 py-2 sm:py-2.5 shadow-[0_8px_32px_rgba(0,0,0,0.06)] flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-2 sm:space-x-2.5 md:space-x-3 group">
            <div className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 bg-gradient-to-br from-black to-neutral-800 rounded-lg sm:rounded-xl flex items-center justify-center transition-all duration-500 group-hover:from-orange-500 group-hover:to-orange-600 shadow-sm shadow-black/10 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-orange-500/25">
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 lg:py-10 relative z-10">
        {children}
      </main>

      <footer className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 border-t border-neutral-50 relative z-10">
      </footer>
    </div>
  )
}
