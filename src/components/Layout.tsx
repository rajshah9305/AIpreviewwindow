...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] bg-[#faf8f6] text-black selection:bg-orange-500/10 selection:text-orange-600 overflow-x-hidden w-full max-w-[100vw]">
      ...
      {/* Desktop top nav */}
      <nav className="sticky top-0 z-[100] w-full px-4 sm:px-4 md:px-6 py-3 sm:py-3 safe-top hidden sm:block">
        <div className="max-w-5xl mx-auto bg-white/90 backdrop-blur-2xl border-2 border-neutral-200 rounded-2xl px-4 md:px-5 py-2.5 md:py-2.5 shadow-[0_8px_40px_rgba(0,0,0,0.06),0_2px_6px_rgba(0,0,0,0.04)] flex items-center justify-between gap-3 transition-all duration-500">
          <Link to="/" className="flex items-center space-x-3 md:space-x-3 group shrink-0 no-underline" style={{ textDecoration: 'none' }}>
            <div className="relative w-10 h-10 md:w-10 md:h-10">
              ...
              <div className="relative w-full h-full bg-gradient-to-br from-neutral-900 to-black rounded-xl flex items-center justify-center transition-all duration-500 group-hover:scale-105 group-hover:shadow-lg group-hover:shadow-orange-500/20 shrink-0">
                <Sparkles className="w-5 h-5 md:w-[18px] md:h-[18px] text-orange-500" />
              </div>
            </div>
            <div className="flex flex-col">
              <h1 className="text-base md:text-[15px] font-display font-extrabold tracking-tighter leading-none text-black whitespace-nowrap" style={{ fontWeight: 800 }}>RAJ AI UI</h1>
              <span className="text-[10px] md:text-[10px] font-display font-semibold text-neutral-400 tracking-wide uppercase leading-none mt-0.5">Component Engine</span>
            </div>
          </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`relative px-4 md:px-4 py-2.5 md:py-2.5 rounded-xl text-xs md:text-xs font-display transition-all duration-300 flex items-center gap-2 md:gap-2 touch-manipulation tracking-tight overflow-hidden no-underline ${
                    active
                ...
                >
                  <Icon className={`w-4 h-4 md:w-4 md:h-4 shrink-0 transition-colors duration-300 ${active ? 'text-orange-400' : ''}`} />
