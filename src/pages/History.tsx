...
const PageHeader = ({ onClearClick }: PageHeaderProps) => (
  <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 sm:gap-4 border-b border-neutral-100 pb-3 sm:pb-4 md:pb-6">
    ...
    <button
      ...
      className="flex items-center justify-center gap-1.5 sm:gap-2 px-4 sm:px-5 py-2 sm:py-2.5 text-red-600 hover:bg-red-50 rounded-lg sm:rounded-xl transition-all text-xs sm:text-sm font-display font-semibold border border-transparent hover:border-red-100 touch-manipulation tracking-tight active:scale-95"
    >
...
const SearchBar = ({ value, onChange }: SearchBarProps) => (
  <div className="relative group">
    ...
    <input
      ...
      className="w-full pl-10 sm:pl-11 pr-3 sm:pr-4 py-3 sm:py-3.5 bg-white border border-neutral-200 rounded-lg sm:rounded-xl text-sm font-sans focus:outline-none focus:border-orange-500/40 focus:ring-4 focus:ring-orange-500/10 transition-all placeholder:text-neutral-400"
...
const HistoryItem = ({ item, isSelected, onSelect }: HistoryItemProps) => (
  ...
  >
    <div className="min-w-0 flex-1">
      <p className={`text-sm sm:text-base font-display font-semibold truncate tracking-tight ${isSelected ? 'text-white' : 'text-black'}`}>
        {item.instruction}
      </p>
      <div className="flex items-center gap-1.5 mt-1.5 opacity-60">
        <Calendar className="w-3 h-3" />
        <p className="text-[10px] sm:text-[11px] font-display font-semibold uppercase tracking-wide">
  ...
  <div className="space-y-4 sm:space-y-6 md:space-y-8 animate-fade-in">
    <div className="p-5 sm:p-7 md:p-9 bg-gradient-to-br from-black to-neutral-900 rounded-xl sm:rounded-2xl md:rounded-3xl text-white relative overflow-hidden shadow-[0_12px_48px_rgba(0,0,0,0.15)]">
      ...
      <div className="relative z-10">
        <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-display font-bold tracking-tight leading-tight">
...
const MetadataBadge = ({ label, value }: { label: string; value: string }) => (
  <div className="px-3 sm:px-4 py-1.5 sm:py-2 bg-white/5 backdrop-blur-sm rounded-lg sm:rounded-xl border border-white/10">
    ...
    <p className="text-[11px] sm:text-xs font-display font-semibold text-white mt-0.5 tracking-tight">{value}</p>
  </div>
)
...
const EmptySelection = () => (
  <div className="h-full min-h-[350px] sm:min-h-[400px] md:min-h-[500px] flex items-center justify-center border-2 border-dashed border-neutral-200 rounded-xl sm:rounded-2xl p-6 sm:p-8 bg-neutral-50/50">
    <div className="text-center">
      ...
      <p className="text-neutral-500 text-xs sm:text-sm font-display font-semibold uppercase tracking-[0.1em]">
        Select a Concept
      </p>
    </div>
  </div>
